...
def _write_travis_input_files(params: TravisECRHInput, output_dir: Path) -> None:
    ...
    ne_central = params.electron_density_1e20[0] if params.electron_density_1e20 is not None else 1.0
    te_central = params.electron_temperature_keV[0] if params.electron_temperature_keV is not None else 3.0
...
Te-parm {te_parm_str}"""
    ...
    target_coords_str = coord_type_map.get(params.target_coords_type, params.target_coords_type)
...
def _write_plasma_profile_file(params: TravisECRHInput, filepath: Path) -> None:
    ...
    if params.rho_grid is None or params.electron_density_1e20 is None or params.electron_temperature_keV is None:
        raise ValueError("Plasma profiles are required")
    
    profile_lines = [f"   {float(rho):18.15f}     {float(ne)*1e20:18.12e}          {float(te):18.12f}                  1.5"
                     for rho, ne, te in zip(params.rho_grid, params.electron_density_1e20, params.electron_temperature_keV)]
...
def _execute_travis(travis_executable: Path, output_dir: Path) -> None:
    ...
    if status_file.exists():
        with open(status_file, 'r') as f:
            ...
            if exit_code != 0:
                raise RuntimeError(f"TRAVIS execution failed with exit code {exit_code}")
...
def _parse_beamtrace(filepath: Path) -> dict:
    ...
    return {
        ...
        'magnetic_field_magnitude_T': data[:, 11],    # Column 12: |B| (needs denormalization)
    }
...
def save_reference_data(output: TravisECRHOutput, filepath: Path) -> None:
    ...
    data = {
        ...
        'linear_power_density_w_per_m': np.asarray(output.linear_power_density_w_per_m).tolist(),
