...
# ---------------------------------------------------------------------------
# RadialProfiles.with_tapered_density
# ---------------------------------------------------------------------------
...
def test_with_tapered_density_monotone_in_taper_region():
...
def test_with_tapered_density_preserves_temperature():
    """Temperature must be unchanged by with_tapered_density."""
    orig = _make_profiles()
    tapered = orig.with_tapered_density(0.1)
...
def test_with_tapered_density_returns_new_object():
    """with_tapered_density must return a new object, not modify in place."""
    orig = _make_profiles()
...
def test_with_tapered_density_invalid_width_raises():
    """Out-of-range boundary_layer_width must raise ValueError."""
    p = _make_profiles()
    with pytest.raises(ValueError):
        p.with_tapered_density(0.0)
    with pytest.raises(ValueError):
        p.with_tapered_density(1.5)  # > rho_max=1
    with pytest.raises(ValueError):
        p.with_tapered_density(-0.1)
